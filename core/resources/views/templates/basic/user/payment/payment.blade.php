    ...
    <script>
        (function($) {
            ...
            $('select[name=gateway]').change(function() {
                ...
                var final_amo = (parseFloat((parseFloat(amount) + parseFloat(charge))) * resource.rate).toFixed(toFixedDigit);
                $('.final_amo').text(final_amo);

                if (resource.currency != '{{ $general->cur_text }}') {
                    var rateElement =
                        `<span>@lang('Conversion Rate')</span> <span><span >1 {{ __($general->cur_text) }} = <span class="rate">${showAmount(resource.rate)}</span>  <span class="base-currency">${resource.currency}</span></span></span>`;
